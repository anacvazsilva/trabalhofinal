...
const_number <- State_Constitutions_Word_Count  %>%
  ...
  select(state, n, constitution_year, current_constitution, statehood, state_number) %>%
  filter(current_constitution == 1) %>%
  mutate(abbreviation = state.abb) %>%
  mutate(regiao = state.region) %>% 
  mutate(current_constitution = NULL, state_number = NULL) %>%
  arrange(desc(n))

const_number$constitution_year <- as.factor(const_number$constitution_year)
 
const_number %>%
  select(abbreviation, n, constitution_year) %>%
  ggplot(aes(x = n, 
